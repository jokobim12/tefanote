...
export default function TransactionList({ transactions, onDelete, onEdit, onClearAll, isFiltered, selectedDate }) {
  ...
  const generatePDF = () => {
    ...
    doc.text("TEFANOTE", 105, 18, { align: "center" });
    
    doc.setFontSize(10);
    doc.setFont("helvetica", "normal");
    doc.text("Laporan Keuangan & Transaksi", 105, 25, { align: "center" });

    // --- INFO SECTION ---
    doc.setTextColor(30, 41, 59);
    doc.setFontSize(10);
    doc.text(`Tanggal Cetak : ${new Date().toLocaleString('id-ID')}`, 14, 50);
    doc.text(`Filter Periode : ${isFiltered ? new Date(selectedDate).toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' }) : 'Semua Transaksi (Hari Ini)'}`, 14, 56);
    doc.text(`Total Transaksi: ${processedData.length} Data`, 14, 62);

    // --- TABLE DATA PREP ---
    const tableColumn = ["No", "Waktu", "Pelanggan", "Rincian Item", "Nominal"];
    const tableRows = [];
    ...
    processedData.forEach((t, index) => {
      ...
      if (t.items && Array.isArray(t.items)) {
          // Format: "- Item Name (2x) : Rp 10.000"
          detailString = t.items.map(i => `• ${i.itemName} (${i.qty}x)`).join('\n');
      } else {
          detailString = `• ${t.itemName} (${t.qty}x)`;
      }
    ...
    autoTable(doc, {
      ...
      columnStyles: {
          ...
          1: { cellWidth: 30 },
          2: { cellWidth: 35 },
          3: { cellWidth: 65 }, // Item details wider
    ...
    doc.roundedRect(120, finalY, 75, 20, 2, 2, 'FD');
    ...
    doc.text(`Rp ${grandTotal.toLocaleString('id-ID')}`, 190, finalY + 14, { align: "right" });
    ...
    for(let i = 1; i <= pageCount; i++) {
        ...
        doc.text("Generated by TefaNote App", 14, 285);
    }

    doc.save(`Laporan_Keuangan_TefaNote_${new Date().toISOString().slice(0,10)}.pdf`);
    toast.success("Laporan PDF berhasil dicetak!");
  };
